...
export default function LecturerDashboard({ setCurrentPage }) {
  ...
  const fullName = user.fullName || user.username || 'Dr. Sarah Chen';
  ...
  const mainNavItems = [
    ...
    { name: 'My Classes', icon: <FiCalendar size={18} /> },
  ...
  const reportNavItems = [
    { name: 'Attendance', icon: <FiBarChart2 size={18} /> },
    { name: 'Reports', icon: <FiFileText size={18} /> },
  ];

  const comingSoonTabs = ['My Classes', 'Book Facility', 'My Students', 'Resources', 'Notifications', 'Attendance', 'Reports'];
            ...
            <div className="space-y-8 max-w-[1200px] mx-auto">
              ...
              <div className="rounded-2xl bg-gradient-to-r from-[#18181b] via-[#27272a] to-[#6a0dad]/80 p-8 text-white relative overflow-hidden shadow-lg border border-gray-800">
                <div className="relative z-10">
                  ...
                  <p className="text-gray-300">Ready for today's lectures?</p>
                </div>
                <div className="absolute top-8 right-8 w-12 h-12 bg-white/10 rounded-xl flex items-center justify-center backdrop-blur-sm border border-white/10">
                  <FiBarChart2 className="text-[#a78bfa]" size={24} />
                </div>
              ...
              <div className="pb-8">
                ...
                <div className="grid grid-cols-3 gap-6">
                  ...
                  <div onClick={() => setActiveTab('My Classes')} className="bg-white rounded-2xl p-8 border-2 border-dashed border-gray-200 hover:border-[#6a0dad] hover:shadow-lg hover:-translate-y-1 transition-all duration-300 text-center flex flex-col items-center cursor-pointer">
                    ...
                    <h4 className="font-syne font-bold text-gray-800 mb-2">My Classes</h4>
                    <p className="text-sm text-gray-400">View your schedule and manage your upcoming lecture sessions.</p>
                  </div>

                  <div onClick={() => setActiveTab('Book Facility')} className="bg-white rounded-2xl p-8 border-2 border-dashed border-gray-200 hover:border-[#6a0dad] hover:shadow-lg hover:-translate-y-1 transition-all duration-300 text-center flex flex-col items-center cursor-pointer">
                    <div className="w-12 h-12 bg-[#6a0dad]/10 rounded-xl flex items-center justify-center mb-4">
                      <FiArchive className="text-[#6a0dad]" size={24} />
                    </div>
                    <h4 className="font-syne font-bold text-gray-800 mb-2">Book Facility</h4>
                    <p className="text-sm text-gray-400">Request labs, lecture halls, or special equipment for your classes.</p>
                  </div>

                  <div onClick={() => setActiveTab('My Students')} className="bg-white rounded-2xl p-8 border-2 border-dashed border-gray-200 hover:border-[#6a0dad] hover:shadow-lg hover:-translate-y-1 transition-all duration-300 text-center flex flex-col items-center cursor-pointer">
                    <div className="w-12 h-12 bg-[#6a0dad]/10 rounded-xl flex items-center justify-center mb-4">
                      <FiUsers className="text-[#6a0dad]" size={24} />
                    </div>
                    <h4 className="font-syne font-bold text-gray-800 mb-2">My Students</h4>
                    <p className="text-sm text-gray-400">Review student details, attendance records, and performance metrics.</p>
                  </div>
          ...
          {comingSoonTabs.includes(activeTab) && (
            <div className="flex flex-col items-center justify-center h-full text-center py-24">
              <div className="w-20 h-20 bg-[#6a0dad]/10 rounded-2xl flex items-center justify-center mb-6">
                <FiArchive className="text-[#6a0dad]" size={36} />
              </div>
